...
import Finances.comdirect_api.types as types
from Finances.comdirect_api.utils import default_callback_p_tan, \
...
class Session:
    ...
    def account_get_balances(self, paging_first=0, paging_count=1000):
        ...
        return [types.AccountBalance(i) for i in response.json()["values"]]
    ...
    def account_get_balance(self, accountid):
        ...
        return types.AccountBalance(response.json())
    ...
    def account_get_transactions(self, accountid, transactionDirection="CREDIT_AND_DEBIT",
        ...
        return [types.AccountTransaction(i) for i in response.json()["values"]]
    ...
    def documents_list(self, uuid=None, paging_first=0, paging_count=1000):
        ...
        response = self._get_authorized(
            ...
            f"paging-first={paging_first}&paging-count={paging_count}")
        return [types.Document(i) for i in response.json()["values"]]
